...
const Carousel_2 = ({ children }) => {
  ...
  const [cardCount, setCardCount] = useState(React.Children.count(children));

  // 카드 개수와 한 번에 보여줄 카드 개수 계산
  useEffect(() => {
    setCardCount(React.Children.count(children));
    // 2개씩 스크롤, 한 번에 2개 보여준다고 가정
    setMaxStartIdx(Math.max(0, React.Children.count(children) - 2));
  }, [children]);
